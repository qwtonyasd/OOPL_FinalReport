...
利用以下四座塔+兩法術去阻擋敵人進入你的領地，如果有怪物到達你的領地，你將會失去生命值，生命值失去到0，關卡過關失敗、1-9血為1星、10-17血為2星、18-20血為3星
...
跟AI討論後 定義出`Unit.hpp`以及`Enemy.hpp`，然後再讓AI依照`Enemy.hpp`去在`EnemyFactory.hpp`做出，哥布林、獸人、狼等重複性高的工作(載入攻擊圖片、走路圖片等等)
## 結語
...
|怪物死掉後還沒播放完動畫而塔還在攻擊他，讓他重複判定 `hp<0`，導致怪物重複進入死亡動畫第一幀 | 血量歸零的瞬間，立刻`m_IsDead = true`，並且讓防禦塔的索敵條件加上 `!enemy->IsDead()`，讓防禦塔去打其他活著的怪物。
|在實做擁有技能的薩滿怪物時，發現只要有薩滿的出現遊戲會閃退，經過排查後發現是跟技能圖片路徑錯誤有關|更改圖片路徑|
|在遊戲外升級選單裡把星星花掉了，進關卡時，防禦塔卻沒有獲得增益|建立獨立的`GameData`單例類別，將6條升級路線與星數獨立出局內生命週期的限制，局外由`UpgradeMenu`寫入天賦等級，局內防禦塔在生成時主動呼叫 `GameData::GetInstance()` 查表函數，將天賦倍率動態乘入基礎傷害與射程中。|
